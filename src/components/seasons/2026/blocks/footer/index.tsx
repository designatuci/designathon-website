...
const footerLinks = [
  ...
  { name: "YouTube", href: "https://www.youtube.com/channel/UC3LqXqBN-R7C91ttlyXHhIQ" },
    ...
    <section className="absolute bottom-0 left-0 right-0">
      <div className="flex flex-col items-center gap-4 p-6 md:flex-row md:items-center md:justify-between md:gap-0 md:p-16">
        <Link
          ...
          className="font-medium text-white transition-colors hover:text-white/90 text-[clamp(0.75rem,2vw,1.5rem)]"
        >
          Design at UCI 2026
        </Link>
        <nav className="flex flex-wrap items-center justify-center gap-3 md:gap-8">
          {footerLinks.map((link) => (
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="font-medium text-white/90 transition-colors hover:text-white text-[clamp(0.75rem,2vw,1.5rem)]"
            >
              {link.name}
            </Link>
          ))}
