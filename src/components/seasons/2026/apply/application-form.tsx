...
const ApplicationForm = ({ onSubmittingChange }: ApplicationFormProps) => {
  ...
  return (
    <>
      ...
      <div
        className={`rounded-2xl border border-white/10 bg-white/[0.04] shadow-2xl backdrop-blur-md transition-all ${submitted ? "py-0" : "p-6 md:p-8"}`}
      >
        {submitted ? (
          <RocketSuccess />
        ) : (
                ...
                },
                {
                  ...
                  text: (
                    <>
                      ...
                      and I must attend Friday, Saturday, and Sunday (in-person
                      or online, but not both) to receive prizes.
                    </>
                  ),
                },
              ].map(({ name, text }) => (
